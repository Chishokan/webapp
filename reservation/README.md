...
### 1. データベースを作成
XServerサーバーパネル →「MySQL設定」で
- データベース(例 `xxxxxxx_yoyaku`)を作成
- ユーザーを作成し、データベースに追加(権限付与)

「phpMyAdmin」を開き、作成したDBを選択 →「インポート」から **`schema.sql`** を実行
(schools / slots / bookings の3テーブルと、7校舎の初期データが入ります)
